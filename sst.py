# MODIFY FOR FINAL PROJECT IMPLEMENTATION----------------------------------------------------
...
import pandas as pd
import nltk
from nltk.tree import Tree         # Should not need in Final Project.

from w266_common import utils, vocabulary
from w266_common import treeviz    # Should not need in Final Project.

# Colors from http://nlp.stanford.edu:8080/sentiment/rntnDemo.html   # Should not need in Final Project.
sst_node_colors = {'0':"#67001f", '1':"#f4a582",
                   '2':"#f7f7f7",
                   '3':"#92c5de", '4':"#052061"}
# Style function to color nodes when visualizing trees.
def sst_node_style(node):                 # Should not need in Final Project.
...
class SSTDataset(object):
    ...
    #Example_fields = ["tokens", "ids", "label", "is_root", "root_id"]  #A2
    Example_fields = ["tokens", "ids", "label"]
    Example = collections.namedtuple("Example", Example_fields)

    @staticmethod
    def get_lines(zipped_filename, split):  # Should not need in Final Project.
        import zipfile
    ...
    @staticmethod
    def parse_line(sexp):                  # Should not need in Final Project.
        if isinstance(sexp, bytes):
            sexp = sexp.decode("utf-8")
        return nltk.tree.Tree.fromstring(sexp)

    def canonicalize(self, raw_tokens):    # Should not need in Final Project.
        wordset=(self.vocab.wordset if self.vocab else None)
        return utils.canonicalize_words(raw_tokens, wordset=wordset)


    def get_trees(self, split):          # Should not need in Final Project.
        return [SSTDataset.parse_line(sexp) for sexp in
                SSTDataset.get_lines(self.zipped_filename, split)]

    def tree_to_example(self, tree, is_root, root_id):     # May need part of this in Final Project.
        tokens = self.canonicalize(tree.leaves())
        ids = self.vocab.words_to_ids(tokens)               ## WILL NEED THIS FUNCTION................
        label = int(tree.label())
        return SSTDataset.Example(tokens, ids, label, is_root, root_id)

    def recursive_extract_phrases(self, tree, output_list, is_root, root_id):     # Should not need in Final Project.
    ...
    def trees_to_dataframe(self, trees):      # Should not need in Final Project.
        examples = []
    ...
    def process(self, label_scheme="binary"):       # Should not need in Final Project.
    ...
    def __init__(self, V=10000):
        self.vocab = None
        #self.zipped_filename = "data/sst/trainDevTestTrees_PTB.zip"     # Should not need in Final Project.
        self.target_names = None  # set by self.process()

        # Download datasets
        '''    # Should not need in Final Project.
        ...
        #self.vocab = vocabulary.Vocabulary(train_words, size=V)  ## NEED TO MODIFY............................
