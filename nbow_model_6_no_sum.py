## Model_6_no_sum: Train using LIAR/Politifact data, GloVe embeddings, AND with LIWC features; then predict on same plus ISOT "title" data also.  These GloVe embeddings are 300 dimensions rather than 50 dims.  DON"T SUM INPUTS.
...
def liwc_features(ids_, V, embed_dim, init_scale=0.001):
    ...
    liwc_type = 'liar_politifact'  # aka, 'lp'
    #liwc_type = 'isot'   
    #liwc_type = 'liar'  # NOTE NEED TO RELOAD MODEL BEFORE EVALUATING WITH A DIFFERENT LIWC EMBEDDING
    
    print('\nLIWC type:', liwc_type, '\n')
    
    if liwc_type == 'liar_politifact':
        liwc_lp = pd.read_pickle('parsed_data/liwc_liarpolitifact.pkl')
        liwc = liwc_lp.astype('float32')    
    elif liwc_type == 'isot':
        liwc_isot = pd.read_pickle('parsed_data/liwc_isot2.pkl')  #74 columns
    ...
    print('V within liwc_features fcn:', V)
    print('embed_dim within liwc_features fcn:', embed_dim)
    print('liwc len:', len(liwc))

    # (this embed dimension does not necessarily match the word embedding dimension)
...
def BOW_encoder(ids_, ns_, V, embed_dim, hidden_dims, dropout_rate=0,
    ...
    with tf.variable_scope("Embedding_Layer"):     
        ...
        liwcs_ = liwc_features(ids_, V, embed_dim2) ### ADD LIWC FEATURE "EMBEDDINGS" IN PARALLEL
    ...
    x_ = tf.reshape(xs_, [-1, 40*embed_dim])   # [batch_size, max_len*embed_dim]
    print('x_.shape:', x_.shape)
    
    liwc_ = tf.reshape(liwcs_, [-1, 40*74])   # [batch_size, max_len*liwc_dim]
    print('liwc_.shape:', liwc_.shape)
