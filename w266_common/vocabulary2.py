...
class Vocabulary2(object):
    ...
    def __init__(self, tokens, size=None,
        ...
        self.embeddings = {v:[] for k,v in self.id_to_word.items()}
    ...
    def set_embeddings(self, word_dic):
        """sets the embeddings from a dictionary."""
        word_keys = word_dic.keys()
        for v,k in self.embeddings.items():
            if(v in word_keys):
                self.embeddings[v] = word_dic[v]
                
    def get_embedding(self, words):
        """gets the  word embeddings"""
        return [self.embeddings.get(w, []) for w in words]
    
    def get_embeddings_all(self):
        """Return a list of embeddings, ordered by id."""
        return self.get_embedding(self.ids_to_words(range(self.size)))
